...
interface PData {
  ...
  totalMenciones: number; totalAlcance: number; totalInteracciones: number; autoresUnicos: number; totalCosto: number;
  sentCounts: { Positivo: number; Negativo: number; Neutro: number }; sentNeto: number;
  trendData: { date: string; total: number; Positivo: number; Negativo: number; Neutro: number; [key: string]: string | number }[];
  fuenteData: { fuente: string; Positivo: number; Negativo: number; Neutro: number; total: number }[];
  kpiByFuente: { fuente: string; menciones: number; interacciones: number; impresiones: number; lideres: number; costo: number }[];
...
function processSocial(rows: SocialRow[]): PData {
  ...
  const dm: Record<string, { date: string; total: number; Positivo: number; Negativo: number; Neutro: number; [key: string]: string | number }> = {};
  rows.forEach(r => { const d = r.Fecha?.substring(0, 10); if (!d) return; if (!dm[d]) dm[d] = { date: d, total: 0, Positivo: 0, Negativo: 0, Neutro: 0 }; dm[d].total++; if (r.Sentimiento === "Positivo" || r.Sentimiento === "Negativo" || r.Sentimiento === "Neutro") dm[d][r.Sentimiento]++; });
  ...
  rows.forEach(r => { const f = r.Fuente || "Otro"; if (!fm[f]) fm[f] = { fuente: f, Positivo: 0, Negativo: 0, Neutro: 0, total: 0 }; fm[f].total++; if (r.Sentimiento === "Positivo" || r.Sentimiento === "Negativo" || r.Sentimiento === "Neutro") fm[f][r.Sentimiento]++; });
  const fd = Object.values(fm).filter(f => f.fuente !== "Otro").sort((a, b) => b.total - a.total);
  const km: Record<string, { fuente: string; menciones: number; interacciones: number; impresiones: number; autores: Set<string> }> = {};
  rows.forEach(r => { const f = r.Fuente; if (!f || f === "Otro") return; if (!km[f]) km[f] = { fuente: f, menciones: 0, interacciones: 0, impresiones: 0, autores: new Set() }; km[f].menciones++; km[f].interacciones += (r.Likes || 0) + (r.Comentarios || 0) + (r.Compartidos || 0); km[f].impresiones += r.Alcance || 0; km[f].autores.add(r.Autor); });
  ...
  rows.forEach(r => { if (!am[r.Autor]) am[r.Autor] = { autor: r.Autor, menciones: 0, alcance: 0, fuente: r.Fuente, interacciones: 0 }; am[r.Autor].menciones++; am[r.Autor].alcance += r.Alcance || 0; am[r.Autor].interacciones += (r.Likes || 0) + (r.Comentarios || 0) + (r.Compartidos || 0); });
  const ta = Object.values(am).sort((a, b) => b.alcance - a.alcance).slice(0, 10);
  const tp = rows.filter(r => r.Sentimiento === "Positivo").sort((a, b) => (b.Alcance || 0) - (a.Alcance || 0)).slice(0, 4).map(r => ({ autor: r.Autor, contenido: r.Contenido || "", link: r.Link || "", fuente: r.Fuente, alcance: r.Alcance || 0 }));
  const tn = rows.filter(r => r.Sentimiento === "Negativo").sort((a, b) => (b.Alcance || 0) - (a.Alcance || 0)).slice(0, 4).map(r => ({ autor: r.Autor, contenido: r.Contenido || "", link: r.Link || "", fuente: r.Fuente, alcance: r.Alcance || 0 }));
  return { dataType: "social", totalMenciones: n, totalAlcance: alc, totalInteracciones: inter, autoresUnicos: uniq, totalCosto: 0, sentCounts: sc, sentNeto: sn, trendData: td, fuenteData: fd, kpiByFuente: kf, topAutores: ta, topPositivas: tp, topNegativas: tn, rawRows: rows };
}

function processMedia(rows: MediaRow[]): PData {
  const n = rows.length;
  const alc = rows.reduce((s, r) => s + (r.Alcance || 0), 0);
  const costTotal = rows.reduce((s, r) => s + (r.Costo || 0), 0);
  const uniq = new Set(rows.map(r => r.Medio)).size;
  const sc: { Positivo: number; Negativo: number; Neutro: number } = { Positivo: 0, Negativo: 0, Neutro: 0 };
  rows.forEach(r => { if (r.Sentimiento === "Positivo" || r.Sentimiento === "Negativo" || r.Sentimiento === "Neutro") sc[r.Sentimiento]++; });
  const sn = n > 0 ? ((sc.Positivo - sc.Negativo) / n) * 100 : 0;
  // Trend by date with per-type breakdown
  const tipos = ["Periódico", "Revista", "Televisión", "Radio", "Internet"];
  const dm: Record<string, Record<string, string | number>> = {};
  rows.forEach(r => {
    const d = r.Fecha?.substring(0, 10); if (!d) return;
    if (!dm[d]) { const entry: Record<string, string | number> = { date: d, total: 0, Positivo: 0, Negativo: 0, Neutro: 0 }; tipos.forEach(t => { entry[t] = 0; }); dm[d] = entry; }
    (dm[d].total as number)++;
    if (r.Sentimiento === "Positivo" || r.Sentimiento === "Negativo" || r.Sentimiento === "Neutro") (dm[d][r.Sentimiento] as number)++;
    const tipo = r.TipoMedio || "Otro";
    if (tipos.includes(tipo)) (dm[d][tipo] as number)++;
  });
  const td = Object.values(dm).sort((a, b) => (a.date as string).localeCompare(b.date as string)) as PData["trendData"];
  const fm: Record<string, { fuente: string; Positivo: number; Negativo: number; Neutro: number; total: number }> = {};
  rows.forEach(r => { const f = r.TipoMedio || "Otro"; if (!fm[f]) fm[f] = { fuente: f, Positivo: 0, Negativo: 0, Neutro: 0, total: 0 }; fm[f].total++; if (r.Sentimiento === "Positivo" || r.Sentimiento === "Negativo" || r.Sentimiento === "Neutro") fm[f][r.Sentimiento]++; });
  ...
  rows.forEach(r => { const f = r.TipoMedio || "Otro"; if (!km[f]) km[f] = { fuente: f, menciones: 0, impresiones: 0, costo: 0, autores: new Set() }; km[f].menciones++; km[f].impresiones += r.Alcance || 0; km[f].costo += r.Costo || 0; km[f].autores.add(r.Medio); });
  ...
  rows.forEach(r => { const k = r.Medio; if (!mm[k]) mm[k] = { autor: k, menciones: 0, alcance: 0, fuente: r.TipoMedio, interacciones: 0 }; mm[k].menciones++; mm[k].alcance += r.Alcance || 0; });
  const ta = Object.values(mm).sort((a, b) => b.alcance - a.alcance).slice(0, 10);
  const tp = rows.filter(r => r.Sentimiento === "Positivo").sort((a, b) => (b.Alcance || 0) - (a.Alcance || 0)).slice(0, 4).map(r => ({ autor: r.Medio, contenido: r.Titulo || (r.Texto ? r.Texto.substring(0, 200) : ""), link: r.Link || "", fuente: r.TipoMedio, alcance: r.Alcance || 0 }));
  const tn = rows.filter(r => r.Sentimiento === "Negativo").sort((a, b) => (b.Alcance || 0) - (a.Alcance || 0)).slice(0, 4).map(r => ({ autor: r.Medio, contenido: r.Titulo || (r.Texto ? r.Texto.substring(0, 200) : ""), link: r.Link || "", fuente: r.TipoMedio, alcance: r.Alcance || 0 }));
  return { dataType: "media", totalMenciones: n, totalAlcance: alc, totalInteracciones: 0, autoresUnicos: uniq, totalCosto: costTotal, sentCounts: sc, sentNeto: sn, trendData: td, fuenteData: fd, kpiByFuente: kf, topAutores: ta, topPositivas: tp, topNegativas: tn, rawRows: rows };
}
...
function aiPicos(d: PData): string {
  ...
  const mx = d.trendData.reduce((m, x) => (x.total as number) > (m.total as number) ? x : m, d.trendData[0]);
...
function aiTendSent(d: PData): string {
  ...
  return "Evolucion: " + ((l.Positivo as number) > (f.Positivo as number) ? "tendencia ascendente en positivas" : "estabilidad en positivas") + ". " + ((l.Negativo as number) > (f.Negativo as number) ? "Negativas en incremento." : "Negativas estables o en descenso.");
}
...
function Donut({ data, brand, title }: { data: { Positivo: number; Negativo: number; Neutro: number }; brand: Brand; title?: string }) {
  ...
  return (<div className="flex flex-col items-center">{title && <div className="text-xs font-bold text-gray-700 mb-1.5">{title}</div>}<PieChart width={160} height={160}><Pie data={cd} cx={80} cy={80} innerRadius={40} outerRadius={68} paddingAngle={3} dataKey="value" stroke="none">{cd.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie></PieChart><div className="flex gap-2 mt-1.5">{cd.map((item, i) => (<div key={i} className="flex items-center gap-1"><div className="w-2 h-2 rounded-sm" style={{ background: item.color }} /><span className="text-[10px] font-bold" style={{ color: item.color }}>{((item.value / total) * 100).toFixed(0)}%</span></div>))}</div></div>);
}
...
export default function Home() {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { const { social, media } = await parseExcel(f); if (social.length) setSData(p => [...p, ...social]); if (media.length) setMData(p => [...p, ...media]); const parts: string[] = []; if (social.length) parts.push(social.length + " redes sociales"); if (media.length) parts.push(media.length + " medios tradicionales"); setUploadStatus(parts.length ? "Cargados: " + parts.join(" y ") : "Sin datos validos"); if (social.length) { setPd(processSocial(social)); setTab("social"); } else if (media.length) { setPd(processMedia(media)); setTab("media"); } } catch { setUploadStatus("Error al procesar."); }
  };
  ...
  const mediaTipoColors = ["#8b5cf6", "#14b8a6", "#ec4899", "#f59e0b", "#3b82f6"];
            ...
            <div className="max-w-[940px] mx-auto p-7 pb-20 grid gap-5">
              ...
              {pd.dataType === "media" && (<>
                ...
                <Sec title="1. Indicadores Clave por Tipo de Medio" icon={<BarChart3 size={18} color={brand.accentColor} />} brand={brand}>
                  ...
                  <div className="grid grid-cols-2 gap-3">{pd.kpiByFuente.map((k, i) => (
                    <div key={i} className="bg-gray-50 rounded-xl p-3.5 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-[10px] font-bold" style={{ background: srcC[k.fuente] || mediaTipoColors[i % 5] }}>{k.fuente.substring(0, 3)}</div>
                ...
                {/* M2. Resumen */}
                <Sec title="2. Resumen Ejecutivo" icon={<FileText size={18} color={brand.accentColor} />} brand={brand}><AIBlock text={aiResumen(pd)} brand={brand} loading={aiLoading} /></Sec>
                {/* M3. Distribucion por Tipo (moved here) */}
                <Sec title="3. Distribucion por Tipo de Medio" icon={<Globe size={18} color={brand.accentColor} />} brand={brand}><ResponsiveContainer width="100%" height={260}><BarChart data={pd.fuenteData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="fuente" tick={{ fontSize: 12, fill: "#64748b" }} /><YAxis tick={{ fontSize: 11, fill: "#94a3b8" }} /><Tooltip /><Bar dataKey="total" radius={[6, 6, 0, 0]} name="Total">{pd.fuenteData.map((e, i) => <Cell key={i} fill={srcC[e.fuente] || mediaTipoColors[i % 5]} />)}</Bar></BarChart></ResponsiveContainer><div className="mt-4"><AIBlock text={aiFuentes(pd)} brand={brand} loading={aiLoading} /></div></Sec>
                {/* M4. Trafico de Informacion */}
                <Sec title="4. Trafico de Informacion" icon={<TrendingUp size={18} color={brand.accentColor} />} brand={brand}>
                  ...
                  <ResponsiveContainer width="100%" height={220}><LineChart data={pd.trendData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="date" tick={{ fontSize: 11, fill: "#94a3b8" }} tickFormatter={(v: string) => v.substring(5)} /><YAxis tick={{ fontSize: 11, fill: "#94a3b8" }} /><Tooltip /><Line type="monotone" dataKey="total" stroke={brand.chartColors[0]} strokeWidth={3} dot={{ fill: brand.chartColors[0], r: 4 }} name="Total" /></LineChart></ResponsiveContainer>
                  <div className="text-xs font-bold text-gray-500 mt-5 mb-2">Por tipo de medio</div>
                  <ResponsiveContainer width="100%" height={220}><LineChart data={pd.trendData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="date" tick={{ fontSize: 11, fill: "#94a3b8" }} tickFormatter={(v: string) => v.substring(5)} /><YAxis tick={{ fontSize: 11, fill: "#94a3b8" }} /><Tooltip /><Legend />
                    <Line type="monotone" dataKey="Periódico" stroke={srcC["Periódico"]} strokeWidth={2} dot={{ r: 3 }} />
                    <Line type="monotone" dataKey="Internet" stroke={srcC["Internet"]} strokeWidth={2} dot={{ r: 3 }} />
                    <Line type="monotone" dataKey="Televisión" stroke={srcC["Televisión"]} strokeWidth={2} dot={{ r: 3 }} />
                    <Line type="monotone" dataKey="Radio" stroke={srcC["Radio"]} strokeWidth={2} dot={{ r: 3 }} />
                    <Line type="monotone" dataKey="Revista" stroke={srcC["Revista"]} strokeWidth={2} dot={{ r: 3 }} />
