...
function AIBlock({text,brand,loading}:{text:string;brand:Brand;loading:boolean}){
  ...
  const renderPart=(t:string)=>{
    ...
    return segs.map((seg,i)=>{
      ...
      const biM=seg.match(/^\*\*\*(.+)\*\*\*$/);
      if(biM)return <strong key={i} className={brand.accentBold?"font-semibold italic":"italic"} style={{color:brand.accentColor}}>{biM[1]}</strong>;
...
FORMATO: JSON válido sin markdown ni backticks. NO uses saltos de línea dentro de valores. Usa " || " para separar párrafos.`;
  ...
  /* ═══ EXPORT: WORD (.docx) — v32 complete rewrite ═══ */
  const exportDOCX=async()=>{
    ...
    try{
      ...
      const{Document:DocxDoc,Packer,Paragraph,TextRun,ImageRun,AlignmentType,PageBreak,
        ExternalHyperlink,PageOrientation,Table,TableRow,TableCell,WidthType,ShadingType}=D;
      ...
      const imgP=(img:{d:Uint8Array;w:number;h:number},maxW=680)=>{
        const r=img.h/img.w;const w=Math.min(maxW,680);const h=Math.round(w*r);
        return new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:100},
          children:[new ImageRun({data:img.d,transformation:{width:w,height:h},type:"png"})]});
      };
      const mkRuns=(text:string)=>{
        ...
        for(const p of parts){
          if(!p)continue;
          const bi=p.match(/^\*\*\*(.+)\*\*\*$/);
          if(bi){runs.push(new TextRun({text:bi[1],bold:brand.accentBold,italics:true,size:24,font:"Arial",color:acHex}));continue}
          const bo=p.match(/^\*\*(.+)\*\*$/);
          if(bo){runs.push(new TextRun({text:bo[1],bold:true,size:24,font:"Arial"}));continue}
          const lk=p.match(/\[Ver fuente\]\(([^)]+)\)/);
          if(lk){runs.push(new ExternalHyperlink({children:[new TextRun({text:" Ver fuente ",style:"Hyperlink",size:20,font:"Arial"})],link:lk[1]}));continue}
          runs.push(new TextRun({text:p,size:24,font:"Arial"}));
        }
        return runs;
      };
      const hd=(text:string)=>new Paragraph({spacing:{before:200,after:80},alignment:AlignmentType.LEFT,
        children:[new TextRun({text,bold:true,size:32,font:"Arial",color:prHex})]});
      const aiParas=(text:string|undefined)=>{
        if(!text)return[];
        const t=typeof text==="string"?text:String(text);
        return t.split(/\s*\|\|\s*/).filter(Boolean).map(p=>
          new Paragraph({spacing:{after:80},alignment:AlignmentType.JUSTIFIED,children:mkRuns(p)}));
      };
      ...
      /* ── Section→AI key mapping ── */
      const aiMap:Record<string,string>={
        "Distribución por Red Social":"fuentes","Distribución por Tipo de Medio":"fuentes","Distribución por Fuente":"fuentes",
        "Tendencia de Conversación":"picos","Tráfico de Información":"picos",
        "Tráfico Redes Sociales":"picosRedes","Tráfico – Redes Sociales":"picosRedes",
        "Tráfico Medios Tradicionales":"picosMedios","Tráfico – Medios Tradicionales":"picosMedios",
        "Actividad por Hora":"hora","Actividad por Hora (redes)":"hora",
        "Análisis de Sentimiento":"sentimiento",
        "Tendencia del Sentimiento":"tendSent",
        "Distribución de Notas por Tier":"tier","Distribución de Notas por Tier (medios)":"tier",
        "Top 5 Medios con Más Noticias":"top5med","Top 5 Medios con Más Noticias (medios)":"top5med",
        "Distribución por Tipo de Nota":"tipoNota","Distribución por Tipo de Nota (medios)":"tipoNota",
        "Distribución por Estado":"estado","Distribución por Estado (medios)":"estado",
        "Top Publicaciones":"topPub","Menciones Populares por Sentimiento":"topPub",
        "Resumen Ejecutivo":"resumen","Conclusiones":"conclusiones","Recomendaciones Estratégicas":"recomendaciones"
      };
      const getAiKey=(title:string):string=>{
        for(const[prefix,key]of Object.entries(aiMap)){if(title.includes(prefix))return key}
        return "";
      };

      /* ── Word table builder for Top5 ── */
      const bdr={style:"single" as const,size:1,color:"E2E8F0"};
      const bds={top:bdr,bottom:bdr,left:bdr,right:bdr};
      const cm={top:50,bottom:50,left:80,right:80};
      const hdrBg={fill:"F8FAFC",type:ShadingType.CLEAR};
      ...
      /* ══════════════════════ BUILD DOCUMENT ══════════════════════ */
      const ch:unknown[]=[];
      const secs=el.querySelectorAll(":scope > div");

      /* PAGE 1: Cover banner */
      if(secs.length>0){
        const bannerImg=await capEl(secs[0] as HTMLElement);
        /* 7.95cm≈226pt, 25.35cm≈719pt → in pixels at 72dpi ≈ 226×3, 719×1 but we use actual ratio */
        const bW=640;const bH=Math.round(640*(bannerImg.h/bannerImg.w));
        /* Center vertically */
        const vPad=Math.max(800,Math.round((5000-bH*5)/2));
        ch.push(new Paragraph({spacing:{before:vPad},children:[new TextRun("")]}));
        ch.push(new Paragraph({alignment:AlignmentType.CENTER,
          children:[new ImageRun({data:bannerImg.d,transformation:{width:bW,height:bH},type:"png"})]}));
      ...
      for(const secEl of secEls){
        ...
        if(secTitle)ch.push(hd(secTitle));

        /* ── Determine section type ── */
        const isTextOnly=["Resumen","Conclusiones","Recomendaciones"].some(k=>secTitle.includes(k));
        const isTemas=secTitle.includes("Temas Clave");
        const isTopPub=secTitle.includes("Top Publicaciones");
        const isMenciones=secTitle.includes("Menciones Populares");
        const isIndicadoresSent=secTitle.includes("Indicadores de Sentimiento");

        if(isTemas){
          /* ── Temas Clave: editable numbered text ── */
          const temas=aiTemas(pd,aiTexts);
          for(let ti=0;ti<temas.length;ti++){
          ...
          /* Per-source tables */
          const sources=pd.dataType==="media"?VM:pd.dataType==="social"?VS:[...VS,...VM];
          for(const src of sources){
            const alcItems=pd.top5AlcPorFuente[src];
            const costItems=pd.top5CostPorFuente[src];
          ...
          if(chartImg)ch.push(imgP(chartImg));
      ...
      /* Footer */
      ch.push(new Paragraph({spacing:{before:200},alignment:AlignmentType.CENTER,
        children:[new TextRun({text:"Generado por ReporteaJDOR",size:16,font:"Arial",color:"94a3b8"})]}));

      const doc=new DocxDoc({
        styles:{default:{document:{run:{font:"Arial",size:24},paragraph:{spacing:{line:240},alignment:AlignmentType.JUSTIFIED}}}},
        sections:[{
          properties:{page:{
            size:{width:15840,height:12240,orientation:PageOrientation.LANDSCAPE},
            margin:{top:720,right:900,bottom:720,left:900}
          }},
          children:ch as InstanceType<typeof Paragraph>[]
        }]
      });
  ...
  const hasAlcData=pd?pd.totalAlcance>0:false;
  const hasCostData=pd?pd.totalCosto>0:false;
          ...
          <div>
            ...
            <div ref={reportContainerRef} id="report-export-target" className="max-w-[940px] mx-auto p-7 pb-20 grid gap-5 overflow-hidden">
              ...
              {pd.dataType==="social"&&(<>                <div style={{order:curOrder.indexOf("Indicadores Clave por Red Social")}}>{sOn("Indicadores Clave por Red Social")&&<Sec title={secNum("Indicadores Clave por Red Social")+". Indicadores Clave por Red Social"} icon={<BarChart3 size={18} color={brand.accentColor}/>} brand={brand}><div className="flex gap-3 flex-wrap mb-4"><KPI title="Menciones" value={pd.totalMenciones} icon={<MessageSquare size={14} color="#3b82f6"/>} color="#3b82f6" brand={brand}/><KPI title="Impresiones" value={fmt(pd.totalAlcance)} icon={<TrendingUp size={14} color="#8b5cf6"/>} color="#8b5cf6" brand={brand}/><KPI title="Interacciones" value={fmt(pd.totalInteracciones)} icon={<Globe size={14} color="#10b981"/>} color="#10b981" brand={brand}/>{pd.totalCosto>0&&<KPI title="AVE" value={fmtM(pd.totalCosto)} icon={<DollarSign size={14} color="#f59e0b"/>} color="#f59e0b" brand={brand}/>}<KPI title="Usuarios" value={pd.autoresUnicos} icon={<Users size={14} color="#ec4899"/>} color="#ec4899" brand={brand}/></div><div className="grid grid-cols-2 gap-3">{pd.kpiByFuente.map((k,i)=>(<div key={i} className="bg-gray-50 rounded-xl p-3.5 flex items-center gap-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{background:srcC[k.fuente]||brand.chartColors[i%7]}}>{socIco[k.fuente]||<span className="text-white text-[10px] font-bold">{k.fuente.substring(0,2)}</span>}</div><div className="flex-1"><div className="text-sm font-bold text-gray-800">{k.fuente}</div><div className="flex gap-2 mt-0.5 flex-wrap"><span className="text-[10px] text-gray-500"><strong>{k.menciones}</strong> pub</span><span className="text-[10px] text-gray-500"><strong>{fmt(k.impresiones)}</strong> alcance</span><span className="text-[10px] text-gray-500"><strong>{fmt(k.interacciones)}</strong> interact</span>{k.costo>0&&<span className="text-[10px] text-gray-500"><strong>{fmtM(k.costo)}</strong> AVE</span>}</div></div></div>))}</div></Sec>}</div>
                ...
                <div style={{order:curOrder.indexOf("Top Publicaciones")}}>{sOn("Top Publicaciones")&&<Sec title={secNum("Top Publicaciones")+". Top Publicaciones"} icon={<Trophy size={18} color={brand.accentColor}/>} brand={brand}><div className="mb-4"><AIBlock text={aiT("topPub",aiTopPub())} brand={brand} loading={aiLoading}/></div><>{hasAlcData&&hasCostData?(<div className="grid grid-cols-2 gap-4 mb-5"><div><div className="text-xs font-bold text-gray-700 mb-2">Top 5 General por Alcance</div><T5Table items={pd.top5AlcGeneral} label="Alcance" brand={brand}/></div><div><div className="text-xs font-bold text-gray-700 mb-2">Top 5 General por {costLabel}</div><T5Table items={pd.top5CostGeneral} label={costLabel} brand={brand}/></div></div>):hasAlcData?(<div className="flex justify-center mb-5"><div className="w-full max-w-lg"><div className="text-xs font-bold text-gray-700 mb-2">Top 5 General por Alcance</div><T5Table items={pd.top5AlcGeneral} label="Alcance" brand={brand}/></div></div>):hasCostData?(<div className="flex justify-center mb-5"><div className="w-full max-w-lg"><div className="text-xs font-bold text-gray-700 mb-2">Top 5 General por {costLabel}</div><T5Table items={pd.top5CostGeneral} label={costLabel} brand={brand}/></div></div>):null}{VS.filter(s=>pd.top5AlcPorFuente[s]?.length>0||pd.top5CostPorFuente[s]?.length>0).map(s=>(<div key={s} className="mb-4"><div className="flex items-center gap-2 mb-2"><div className="w-6 h-6 rounded-lg flex items-center justify-center" style={{background:srcC[s]}}>{socIco[s]}</div><span className="text-sm font-bold text-gray-800">{s}</span></div><div className="grid grid-cols-2 gap-4">{hasAlcData&&pd.top5AlcPorFuente[s]?.length>0&&<div><div className="text-[11px] font-semibold text-gray-500 mb-1">Por Alcance</div><T5Table items={pd.top5AlcPorFuente[s]} label="Alcance" brand={brand}/></div>}{hasCostData&&pd.top5CostPorFuente[s]?.length>0&&<div><div className="text-[11px] font-semibold text-gray-500 mb-1">Por {costLabel}</div><T5Table items={pd.top5CostPorFuente[s]} label={costLabel} brand={brand}/></div>}</div></div>))}</></Sec>}</div>
              ...
              {pd.dataType==="media"&&(<>                <div style={{order:curOrder.indexOf("Indicadores Clave por Tipo de Medio")}}>{sOn("Indicadores Clave por Tipo de Medio")&&<Sec title={secNum("Indicadores Clave por Tipo de Medio")+". Indicadores Clave por Tipo de Medio"} icon={<BarChart3 size={18} color={brand.accentColor}/>} brand={brand}><div className="flex gap-3 flex-wrap mb-4"><KPI title="Total Noticias" value={pd.totalMenciones} icon={<MessageSquare size={14} color="#3b82f6"/>} color="#3b82f6" brand={brand}/>{pd.totalAlcance>0&&<KPI title="Alcance Total" value={fmt(pd.totalAlcance)} icon={<TrendingUp size={14} color="#8b5cf6"/>} color="#8b5cf6" brand={brand}/>}{pd.totalCosto>0&&<KPI title="Costo Total" value={fmtM(pd.totalCosto)} icon={<DollarSign size={14} color="#10b981"/>} color="#10b981" brand={brand}/>}<KPI title="Medios" value={pd.autoresUnicos} icon={<Users size={14} color="#f59e0b"/>} color="#f59e0b" brand={brand}/></div><div className="grid grid-cols-2 gap-3">{pd.kpiByFuente.map((k,i)=>(<div key={i} className="bg-gray-50 rounded-xl p-3.5 flex items-center gap-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{background:brand.mediaColors[k.fuente]||srcC[k.fuente]||brand.chartColors[i%7]}}>{medIco[k.fuente]||<span className="text-white text-[10px] font-bold">{k.fuente.substring(0,3)}</span>}</div><div className="flex-1"><div className="text-sm font-bold text-gray-800">{k.fuente}</div><div className="flex gap-2 mt-0.5 flex-wrap"><span className="text-[10px] text-gray-500"><strong>{k.menciones}</strong> notas</span>{k.impresiones>0&&<span className="text-[10px] text-gray-500"><strong>{fmt(k.impresiones)}</strong> alcance</span>}{k.costo>0&&<span className="text-[10px] text-gray-500"><strong>{fmtM(k.costo)}</strong> costo</span>}<span className="text-[10px] text-gray-500"><strong>{k.lideres}</strong> medios</span></div></div></div>))}</div></Sec>}</div>
                ...
                <div style={{order:curOrder.indexOf("Top Publicaciones")}}>{sOn("Top Publicaciones")&&<Sec title={secNum("Top Publicaciones")+". Top Publicaciones"} icon={<Trophy size={18} color={brand.accentColor}/>} brand={brand}><div className="mb-4"><AIBlock text={aiT("topPub",aiTopPub())} brand={brand} loading={aiLoading}/></div><>{hasAlcData&&hasCostData?(<div className="grid grid-cols-2 gap-4 mb-5"><div><div className="text-xs font-bold text-gray-700 mb-2">Top 5 General por Alcance</div><T5Table items={pd.top5AlcGeneral} label="Alcance" brand={brand} showTitulo/></div><div><div className="text-xs font-bold text-gray-700 mb-2">Top 5 General por Costo</div><T5Table items={pd.top5CostGeneral} label="Costo" brand={brand} showTitulo/></div></div>):hasAlcData?(<div className="flex justify-center mb-5"><div className="w-full max-w-lg"><div className="text-xs font-bold text-gray-700 mb-2">Top 5 General por Alcance</div><T5Table items={pd.top5AlcGeneral} label="Alcance" brand={brand} showTitulo/></div></div>):hasCostData?(<div className="flex justify-center mb-5"><div className="w-full max-w-lg"><div className="text-xs font-bold text-gray-700 mb-2">Top 5 General por Costo</div><T5Table items={pd.top5CostGeneral} label="Costo" brand={brand} showTitulo/></div></div>):null}{VM.filter(t=>pd.top5AlcPorFuente[t]?.length>0||pd.top5CostPorFuente[t]?.length>0).map(t=>(<div key={t} className="mb-4"><div className="flex items-center gap-2 mb-2"><div className="w-6 h-6 rounded-lg flex items-center justify-center" style={{background:brand.mediaColors[t]||srcC[t]}}>{medIco[t]}</div><span className="text-sm font-bold text-gray-800">{t}</span></div><div className="grid grid-cols-2 gap-4">{hasAlcData&&pd.top5AlcPorFuente[t]?.length>0&&<div><div className="text-[11px] font-semibold text-gray-500 mb-1">Por Alcance</div><T5Table items={pd.top5AlcPorFuente[t]} label="Alcance" brand={brand} showTitulo/></div>}{hasCostData&&pd.top5CostPorFuente[t]?.length>0&&<div><div className="text-[11px] font-semibold text-gray-500 mb-1">Por Costo</div><T5Table items={pd.top5CostPorFuente[t]} label="Costo" brand={brand} showTitulo/></div>}</div></div>))}</></Sec>}</div>
                <div style={{order:curOrder.indexOf("Conclusiones")}}>{sOn("Conclusiones")&&<Sec title={secNum("Conclusiones")+". Conclusiones"} icon={<FileText size={18} color={brand.accentColor}/>} brand={brand}><AIBlock text={aiT("conclusiones",aiConc(pd))} brand={brand} loading={aiLoading}/></Sec>}</div>
                <div style={{order:curOrder.indexOf("Recomendaciones Estratégicas")}}>{sOn("Recomendaciones Estratégicas")&&<Sec title={secNum("Recomendaciones Estratégicas")+". Recomendaciones Estratégicas"} icon={<Lightbulb size={18} color={brand.accentColor}/>} brand={brand}><AIBlock text={aiT("recomendaciones",aiRec(pd))} brand={brand} loading={aiLoading}/></Sec>}</div>
              </>)}
